...
const { createAuction, clearAuctions } = require('../auction/auction.factory')
const { createBid, clearBids } = require('./bid.factory')
...
describe('Bid', () => {
  ...
  it.each([{ type: 'public' }, { type: 'authenticated' }])(
    ...
    async ({ type }) => {
      ...
      const req = request(strapi.server.httpServer)
        .get(`/api/bids/auction/${primaryAuction.id}/latest`)
  ...
  it('should authenticated user be able to do a bid for asc auction', async () => {
    ...
    await request(strapi.server.httpServer)
      .post(`/api/bids/auction/${auction.id}`)
      .set('accept', 'application/json')
      .set('Content-Type', 'application/json')
